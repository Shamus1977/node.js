...
.post(authenticate.verifyUser, (req, res) => {
    res.statusCode = 404;
    res.end(`POST operation not permitted on /campsites/${req.params.campsiteId}/comments/${req.params.commentId}`);
})
.put(authenticate.verifyUser, (req, res, next) => {
    ...
    .then(campsite => {
        if(campsite.comments.id(req.params.commentId).author.equals(req.user._id)){
            ...
            const err = new Error('You are not authorized to update.');
            err.status = 403;
            return next(err);
        }
    })
